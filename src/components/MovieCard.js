...
const MovieCard = ({title, poster_path, overview, vote_average, id}) => {
  ...
  return (
  <div className="movie" onClick={()=> currentUser ? navigate("/details/"+id): alert("Please log in to see in details")}>
    ...
    <div className="d-flex align-items-baseline justify-content-between p-1 text-white">
      <h5>{title}</h5>
      {currentUser && (<span className= { `tag ${vote_average >= 8 ? "green": vote_average >= 7 ? "orange" :"red" }`}>{vote_average}</span>)}
